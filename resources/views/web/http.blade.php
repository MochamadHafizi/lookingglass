...
<section>
    <div class="flex justify-center" style="margin-top: 5em; margin-left: 5em;">	
        <form class="w-3/4 shadow-md my-5 py-2 px-2 bg-gray-50" method="POST" action="{{route('web.httphc')}}">
            @csrf             
            <h3 class="font-black text-xs text-orange-400">HTTP Header Checker</h3>
            <div class="flex items-center justify-center space-x-2">
              ...
              <div class="relative z-0 w-full group flex-2">
                ...
                <label for="domain" style="font-size: 11px;" class="peer-focus:font-medium absolute text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-orange-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">DomainL</label>
            ...
            <table class="min-w-full bg-white border rounded-lg shadow-lg text-xs">
              <tbody>
                  @foreach ($resultArray as $key => $value)
                      @if (!in_array($key, ['timestamp', 'apiVersion', 'apiStatus', 'apiCode']))
                          <tr>
                              <td class="border px-4 py-2">{{ str_replace([':', '"', ','], '', $key) }}</td>
                              <td class="border px-4 py-2">
                                  @if (is_array($value) || is_object($value))
                                      <pre class="whitespace-pre-wrap">{{ str_replace(['{', '}', '[', ']', '"', ','], '', json_encode($value, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)) }}</pre>
                                  @else
                                      {{ str_replace(['"', ':', ','], '', $value) }}
                                  @endif
                              </td>
